...
final class CaptureViewModel: ObservableObject {
    ...
    private func parse(observations: [DonutTextObservation]) -> (normalizedFields: NormalizedFields?, fields: [OCRCandidateField]) {
        ...
        let maxObservations = 40
        let limitedObservations = Array(observations.prefix(maxObservations))
        let allText = limitedObservations.map { $0.text }.joined(separator: "\n")
        let lines = allText
            .components(separatedBy: CharacterSet.newlines)
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }

        var candidateBuckets: [OCRCandidateField.FieldType: [OCRCandidateField]] = [:]
        var seenByType: [OCRCandidateField.FieldType: Set<String>] = [:]
        let maxSuggestionsPerField = 6
        ...
        func register(_ value: String, for type: OCRCandidateField.FieldType, confidence: Double) {
            ...
            var bucket = candidateBuckets[type, default: []]
            bucket.append(OCRCandidateField(type: type, value: value, confidence: confidence))
            bucket.sort { $0.confidence > $1.confidence }
            if bucket.count > maxSuggestionsPerField {
                bucket.removeLast(bucket.count - maxSuggestionsPerField)
            }
            candidateBuckets[type] = bucket
        }

        for (index, line) in lines.enumerated() {
            let confidence = Double(limitedObservations[min(index, limitedObservations.count - 1)].confidence)
            if index == 0 {
        ...
        if let regex = try? NSRegularExpression(pattern: pattern, options: []) {
            ...
            regex.enumerateMatches(in: loweredText, options: [], range: range) { match, _, _ in
                ...
                guard volumeCandidates.count < maxSuggestionsPerField else { return }
        ...
        let candidateFields = OCRCandidateField.FieldType.allCases.flatMap { candidateBuckets[$0] ?? [] }

        let bestName = candidateBuckets[.name]?.first?.value.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !bestName.isEmpty else {
            return (nil, candidateFields)
        }
        let bestSubName = candidateBuckets[.subName]?.first?.value.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        let typeValue = candidateBuckets[.type]?.first?.value ?? (inventoryService.availableTypes.last ?? "Other")
